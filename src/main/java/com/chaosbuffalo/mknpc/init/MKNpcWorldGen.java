...
public class MKNpcWorldGen {
    ...
    public static StructurePieceType TEST_PIECE_TYPE;
    public static StructurePieceType TEST_JIGSAW_PIECE_TYPE;
...
//    private static ConfiguredStructureFeature<?, ?> TEST_STRUCTURE_FEATURE;
    ...
    private static ConfiguredStructureFeature<?, ?> TEST_JIGSAW_FEATURE;
    public static List<StructureFeature<?>> NO_WATER_STRUCTURES = new ArrayList<>();
    public static StructurePieceType MK_JIGSAW_PIECE_TYPE;
...
//        TEST_STRUCTURE_FEATURE = TEST_STRUCTURE.configured(new ChunkPosConfig(0, 0));
//        StructureFeature.STRUCTURES_REGISTRY.put(TEST_STRUCTURE_NAME.toString(), TEST_STRUCTURE);
//        StructureFeature.STEP.put(TEST_STRUCTURE, GenerationStep.Decoration.SURFACE_STRUCTURES);
//        evt.getRegistry().register(TEST_STRUCTURE);

        TEST_JIGSAW = new MKJigsawStructure(JigsawConfiguration.CODEC, 0, true, true, false);
        TEST_JIGSAW.setRegistryName(TEST_JIG_SAW_NAME);
        StructureFeature.STRUCTURES_REGISTRY.put(TEST_JIG_SAW_NAME.toString(), TEST_JIGSAW);
        StructureFeature.STEP.put(TEST_JIGSAW, GenerationStep.Decoration.SURFACE_STRUCTURES);
        TEST_JIGSAW_FEATURE = TEST_JIGSAW.configured(new JigsawConfiguration(
    ...
    public static void registerFeatures(RegistryEvent.Register<Feature<?>> evt){
        SPRING_REPLACEMENT = new MKSpringFeature(SpringConfiguration.CODEC);
...
//        event.getServer().registryAccess().getRegistry(Registry.NOISE_SETTINGS_KEY).forEach(dimensionSettings -> {
//            dimensionSettings.getStructures().structureConfig().put(TEST_STRUCTURE, new StructureSeparationSettings(2, 1, 34222645));
//            dimensionSettings.getStructures().structureConfig().put(TEST_JIGSAW, new StructureSeparationSettings(10, 5, 32441244));
//        });
    }
